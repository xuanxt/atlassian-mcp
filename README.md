...
- Issue votes
- Attachment upload (only download/metadata supported)
- Advanced roadmap features
- Portfolio/Advanced Roadmap APIs
...
**Advanced Agile**:
- Release management beyond versions
- Custom sprint fields
- Board-level permissions
- Rapid board configuration
...
1. **Protect API Token**
   - API tokens are equivalent to passwords
   - Store securely, never commit to version control
   - Never share in public repositories

2. **Regular Rotation**
   - Rotate API tokens periodically
   - Immediately revoke and regenerate if compromised

3. **Least Privilege Principle**
   - Grant only necessary permissions
   - Consider creating a dedicated Atlassian account for MCP server

4. **Secure Configuration**
   - Keep `claude_desktop_config.json` file permissions restricted
   - Review and audit tool usage regularly
